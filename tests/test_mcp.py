...
def test_mcp_module_imports_and_exposes_app():
    ...
    from capture_helper import mcp as mcp_module

    assert hasattr(mcp_module, "app"), "capture_helper.mcp must re-expose `app`."
    assert hasattr(mcp_module, "mcp"), "capture_helper.mcp must expose the `mcp` handler."


def test_main_entrypoint_is_callable():
    """The ``capture-helper-mcp`` console entry point should be a callable."""
    from capture_helper.mcp import main
